...
def explore_environment(env):
    ##### EXPLORATION CODE TO GET FAMILIAR WITH THE ENVIRONMENT
    # 1. Reset the environment to get the first observation
    # This calls your agent.make_features method internally
    obs, info = env.reset()

    current_df = env.unwrapped.df
    print("--- Environment dataframe ---")
    print(current_df.head()) # Shows the first 5 rows
    print(current_df.head().shift(1))
    print(current_df.head() / current_df.head().shift(1))

    print("--- Environment Observation Space ---")
    print(f"Observation Shape: {obs.shape}")
    # These are the features you created in make_features
    print(f"Observation (First Sample): {obs[0]}") 
    print(f"Observation (Full): {obs}") 

    print("\n--- Action Space ---")
    # This comes from your agent.get_position_list()
    print(f"Available Actions: {env.action_space}")

    # 2. Take a dummy step to see what the environment returns
    # Action 0 usually corresponds to the first item in your get_position_list
    action = 0 
    ...
    # The 'info' dict often contains 'total_reward', 'portfolio_valuation', etc.
    print(f"Current Portfolio Value: {info.get('portfolio_valuation')}")
